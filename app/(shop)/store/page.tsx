...
export default async function Store(props: { 
    ...
    const searchParams = await props.searchParams; //Data from url 
    const allGames = await getCachedGames(); //games form cache
    ...
    const filteredGames = allGames.filter(game => {
        ...
        // if the game is not in price bar, throw it
        ...
        if (ratings.length > 0) {
            ...
            const matchesRating = ratings.some(star => { 
                ...
                if (star === 4){
                    return rating >= 6.1 && rating < 8.1; 
                }
                if (star === 3){
                    return rating >= 4.1 && rating < 6.1;
                } 
                if (star === 2){
                    return rating >= 2.1 && rating < 4.1;
                } 
                if (star === 1){
                    return rating >= 0 && rating < 2.1;
                } 
                return false;
            });
            if (!matchesRating) {
        ...
        if (genres.length > 0) {
            ...
            if (!genres.some(g => gameGenreNames.includes(g))){
                return false;
            } 
        }
        ...
        if (platforms.length > 0) {
            ...
            if (!platforms.some(p => gamePlatforms.includes(p))) {
                return false;
            }
        }
    ...
    const sortedGames = [...filteredGames].sort((a, b) => {
        switch (sortType) {
            ...
            case 'PriceDown': {
               return Number(b.price_eur) - Number(a.price_eur); 
            }
            case 'PopularityUp': {
                return Number(a.rating_summary || 0) - Number(b.rating_summary || 0);
            }
            case 'PopularityDown': {
               return Number(b.rating_summary || 0) - Number(a.rating_summary || 0); 
            }
